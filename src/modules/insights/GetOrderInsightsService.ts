...
/**
 * Sends a restaurant's full order history to OpenAI and asks it to produce
 * structured business insights. Note: this forwards real operational data
 * (table numbers, prices, observations) to a third-party API with no
 * redaction — see RFC §"Third-party data exposure" before extending this.
 */
export class GetOrderInsightsService {
  ...
  /**
   * Fetches all orders for `restaurantId` and asks the model for
   * insights/trends/recommendations/metrics. DB and OpenAI failures are left
   * uncaught — the controller logs them with full request context via
